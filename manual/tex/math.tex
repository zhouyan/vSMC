...
(except \verb|sincos| and \verb|modf| which have two output parameters). For
all functions, the output is always a vector. If there are more than one input
parameters, then some of them, but not all, can be scalars. For example, for
the function call \verb|fma(n, a, b, c, y)| in Table~\ref{tab:Arithmetic
...
parameter \verb|y| has to be a pointer to a vector. Therefore, there are $2^3 -
1$ versions of this function for each type of the value. Note that, mixed
