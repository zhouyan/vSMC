        ...
        const T *first)
{
    ...
    hid_t dataset = H5Dcreate(datafile, data_name.c_str(),
        ...
        T *first)
{
    ...
    hdf5_write_matrix<Order>(nrow, ncol, data_name, file_name, cfirst);
        ...
        InputIter first)
{
    ...
    for (std::size_t i = 0; i != N; ++i, ++first)
        data[i] = *first;
    hdf5_write_matrix<Order>(nrow, ncol, data_name, file_name, &data[0]);
        ...
        InputIterIter first)
{
    ...
    hdf5_write_matrix<ColMajor>(nrow, ncol, data_name, file_name, &data[0]);
}
...
inline void hdf5_write_data_frame (std::size_t nrow, std::size_t ncol,
        const std::string &data_name, const std::string &file_name,
        InputIterIter first, SInputIter sfirst)
{
