...
\begin{cppcode}
  ...
  r1 == r2; // always true
\end{cppcode}
The second overloading version of \verb|rng_rand| is similar to the loop,
\begin{cppcode}
  for (std::size_t i = 0; i != n; ++i)
...
generate the same 256-bits patterns. The only difference is that
\verb|operator()| of the former returns 32-bits integers while the later
...
\begin{table}
  ...
  \begin{tabularx}{\textwidth}{Xrr}
    ...
    & $128$  & $192$  \\
    \texttt{AES256x1}, \texttt{ARS256x2}, \texttt{AES256x4}, \texttt{AES256x8}
    & $128$  & $256$  \\
    \texttt{ARSx1}, \texttt{ARS256x2}, \texttt{ARSx4}, \texttt{ARSx8}
    & $128$  & $128$  \\
...
More generally the library defines the following class,
\begin{cppcode}
...
\verb|sizeof(ResultType)|. This rule will enforced at compile-time. In the
remaining of this section, we introduce a few generators implemented by this
class. A few configuration macros of these \rng{}s are listed in
...
multiple 128-bits random integers are generated at the same time. This is
specified by the template parameter \verb|Blocks|. Larger blocks, up to eight,
can improve performance but this is at the cost of larger state size. Without
...
\textcite{Salmon:2011um}. The default number of blocks can be changed through
the configuration macros. The default values are \verb|8|. The default number
of rounds of the \ars engine can also be changed by the configuration macro.
The default value is \verb|5|. The last template parameter \verb|Constants| of
...
defaults are taken from the paper. To use an alternative set 64-bits integers
for the constants, one can define and use a trait class as the following,
\begin{cppcode}
  template <std::size_t>
...
A few type alias are defined for convenience. For example,
\begin{cppcode}
...
\end{cppcode}
The engine \verb|ARS| is the library's default \rng if \aesni is supported.
Aliases for block sizes 1, 2, 4, 8 are defined for all four algorithms, as well
as both 32- and 64-bits versions. These aliases are listed in
table~\ref{tab:Counter-based RNG}.

The performance of these engines depend on a few factors, such as \cpu types,
...
32-, and 64-bits random integers, respectively.
...
\section{RNG in C and OpenCL}
\label{sec:RNG in C and OpenCL}

The Philox and Threefry engines, together with a set of functions that transfer
random integers to floating points numbers, are also implemented with only a
subset of \cnn features. They can be used in both \cnn and OpenCL 1.2 device
programs. See appendix~\appref{app:sub:Parallelized implementation using
...
random integers. These functions take the form of the engine name suffixed with
\verb|_rand|. They take a pointer to the \rng as input and return an unsigned
integer.

The functions that convert random integers to floating points uniform on $[0,
1]$ take the form
\texttt{vsmc\_u01\_%
