...
\end{cppcode}
In the above program, the \cppinline{MKLSSTask} type object manages a
\cppinline{VSLSSTaskPtr} task pointer. The resouces managed will be released
...
\cppinline{cl_platform}. The resources will be released when the object is
destroyed. Note that, the copy constructor and assignment operator perform
shadow copy. This is particularly important for \cppinline{CLMemory} type
objects. In appendix~\appref{app:sub:Parallelized implementation using OpenCL}
