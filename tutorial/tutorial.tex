...
similarity to the aforementioned \smctc, which is by intention. For example,
they both use user defined callbacks to perform application specific
...
duplicate work need to be done for different algorithms. Both flexibility and
better performance are made possible through the use of template
metaprogramming, a technique pioneered by \cblitz. Some differences of the two
...
comparison can be found in Appendix~\ref{sec:vsmc and smctc}. We will not
...
influences the estimator variance. See \cite{DelMoral:2006hc} for further
...
\begin{description}
  ...
  \item[Message Passing Interface] \mpi is the \emph{de facto} standard for
    ...
    such that the same sampler can be parallelized using \mpi. In addition,
    when used with the \smp module, it allows easy implementation of hybrid
    parallelization such as \mpi/\openmp. In Section~\ref{sub:Bayesian
  ...
  \item[OpenCL] This module is similar to the two above except it eases the
    ...
    programs that can be executed across heterogeneous platforms. \opencl
    programs can run on either \cpu{}s or \gpu{}s. It is beyond the scope of
    this paper to give a proper introduction to \gpgpu, \opencl and their use
...
the headers inside the \cppinline{include} directory. To permanently install
the library in a system directory, one can simply copy the contents of the
\cppinline{include} directory to an appropriate place.
...
The source of examples are in the \cppinline{vSMCExample} directory. To build
...
\cppinline{make} from \cppinline{example} to \cppinline{paper}. The source of
these two examples are in the \cppinline{paper} subdirectory of the
\cppinline{vSMCExample} directory.
...
manual by invoking \cppinline{make docs}. The latest reference manual can also
...
with classes defined in \cppoo{} \cppinline{<random>} or their \cboost
...
to standard compliant and feature completion. \gcc 4.8 , \msvc 2012 and \icpc
2013 also have very good \cppoo support. Note that, provided the \boost
...
significantly better. From the author's experience, \clang 3.3, \gcc 4.7,
\icpc 2010 and \msvc 2010, or later versions of these compilers are
...
\begin{itemize}
  ...
  \item A collection of all particle state values and their associated
    ...
    By default this is an \cppinline{RngSet<Threefry4x32, Vector>} object.
...
\vsmc and \smctc. In the later library, users define classes that abstract
...
\end{cppcode}
If \cppinline{size_type} is found inside a value collection type \cppinline{T}, \vsmc
classes such as \cppinline{Particle<T>} will define their own
\cppinline{size_type} to exactly the same type. Since \vsmc allows one to
...
using \cpp template programming. The actual type of the pointer
...
supports function calling syntax can be used. Unlike \smctc, only function
...
double r = rnorm(particle.rng(i));
\end{cppcode}
\vsmc itself also uses of \cppoo{} \cppinline{<random>} or \boost depending on
...
\cppinline{<random>}, it is imported to the \cppinline{cxx11} namespace.
...
either \cppoo standard library or \boost. Note that, \vsmc defines classes
that are available in \cppoo{} \cppinline{<random>} but not in \boost inside
the \cppinline{cxx11} namespace if \boost is used, but not \emph{vice versa}.
The benefit is that if one needs to develop on multiple platforms, and only
some of them support \cppoo and some of them have the \boost library, then
only the configuration macro \cppinline{VSMC_HAS_CXX11LIB_RANDOM} needs to be
changed. This can be configured through \cmake and other build systems. Of
...
same for each variable.  This is similar to \crlang's \cppinline{data.frame}.
...
\end{cppcode}
where \cppinline{cal_inc_weight} is some function that calculates the logarithm
incremental weights. As we see, there are three main parts of a typical move.
First, we allocate a vector named \cppinline{inc_weight}. Second, we generate
normal random variates for each particle and calculate the incremental
weights. This is done through a \cppinline{for} loop. Third, we add the
logarithm incremental weights. The first and the third are \emph{global}
operations while the second is \emph{local}. The first and the third are often
optional and absent. The local operation is also usually the most
computational intensive part of \smc algorithms and can benefit the most from
parallelization.
...
\end{cppcode}
The \cppinline{operator()} of \cppinline{MoveSEQ<T>} is equivalent to the single
...
class name causes \vsmc to use \openmp to parallelize the loop. For example,
...
\cppinline{move::operator()} is called, it will be parallelized by \openmp.
Other backends are also supported in case that \openmp is not available. Among
them there are \ccilk and \tbb. In addition to these well known
parallelization programming models, \vsmc also has its own implementation
using \cppoo{} \cppinline{<thread>}. There are other backends documented in
the reference manual.  To use any of them, all one needs to do is to change a
...
few exceptions,
\begin{itemize}
  \item The constructors of \cppinline{Particle} and \cppinline{Sampler} are not
    thread-safe. Therefore if one need to construct multiple \cppinline{Sampler}
...
different threads. For example, \cppinline{WeightSet::set_log_weight} cannot be
called from multiple threads on the same object.
...
\cppinline{post_processor} does not have to be thread-safe, but
...
The class \cppinline{cv} will be our value collection which is a subclass of
\cppinline{StateMatrix<RowMajor, Dim, double>}. To illustrate both the core
module and the \smp module, the initialization \cppinline{cv_init} will be
implemented as a standalone function. The move \cppinline{cv_move} will be
implemented as a derived class of \cppinline{MoveSEQ<cv>}. To monitor the
importance sampling estimates of the two position parameters, we will
implement a simple function \cppinline{cv_monitor_state} and use the adapter
...
\rlang, and get the plot of the estimates of positions against the
observations, as shown in Figure~\ref{fig:pf}.

For people familiar with the \smctc library, the implementation demonstrated
...
in \smctc. However, now we can benefit from parallelization. In addition, even
the sequential version runs about twice faster than that of \smctc. The
object-oriented design of \vsmc also allows better and safer implementations.
For example, in \smctc, to pass informations to a callback, one can only do
...
parallelized samplers. For example, when using \openmp parallelization, the
...
distributed source, this is configured by the \cmake build system.
...
the \cppinline{gmm_param} class. We omitted definitions of some data access
member functions.
\begin{cppcode}
class gmm_param
{
...
\vsmc's \mpi module assumes that identical samplers are constructed on each
...
for use with \mpi, first at the beginning of the \cppinline{main} function, we
add the following,
\begin{cppcode}
MPIEnvironment env(argc, argv);
\end{cppcode}
to initialize the \mpi environment. When the object \cppinline{env} is
destroyed at the exit of the \cppinline{main} function, the \mpi environment
...
\cppinline{gmm_param} class now needs to be transferable using \mpi. Unlike
the \smp situations, a simple copy constructor is not enough. \vsmc uses
\boost{} \mpi library, and thus one only needs to write a
\cppinline{serialize} member function for \cppinline{gmm_param} such that the
data can be serialized into bytes. See documents of \boost{} \mpi and
...
After these few lines of change, the sampler is now parallelized using \mpi
...
multi-threading parallelization locally. \vsmc's \mpi module will take care of
normalizing weights and other tasks.
...
run on both \cpu{}s and \gpu{}s. For \cpu implementation, there are \ciocl and
\caocl platforms. We use the \tbb implementation as a baseline for comparison.
...
a very different design to that of \smctc. It is more modular and almost all
...
\smctc, if it is not provided by the library, users have to implement all the
...
particle. And in the \cppinline{Sampler}'s constructor, one pass it this object
instead of the name of the built-in resampling scheme name. \vsmc's
...
example is of interest when the sampler is parallelized using \mpi, where
...
do is providing \vsmc with a specialized \cppinline{copy} member function inside
their value collection class. And the implementation of the resampling can be
reused. (This is exactly what \vsmc does for the \opencl and \mpi module.)

In some situations, \smctc might appear to be easier to use than \vsmc. For
...
type.  Alternatively, one can also reuse classes defined for \smctc by
...
\mcmc algorithms, such as those used in \cite{Calderhead:2009bd} for Bayesian
model comparison. In addition, when multiple algorithms share common
...
faster than that in \smctc.

In the author's opinion, \smctc is suitable for implementing algorithms that
